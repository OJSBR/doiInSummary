 ...
 *
 * Functional tests: the plugin can be enabled, and the DOI a reader sees in an
 ...
 * integration, and the first test enables the plugin when it is off. The reader
 * test also needs withDois=1 and a published issue whose articles have DOIs
 * (issuePath, default "issue/current"), which the CI data set does not have.
 */
...
describe('DOI in Summary plugin', function() {
	...
	const withDois = Cypress.env('withDois');
	...
	it('Enables the plugin', function() {
		...
		openPluginsTab();
		enablePlugin('doiinsummaryplugin');
	});

	(withDois ? it : it.skip)('Shows each DOI once, as a resolving link, right under the article title', function() {
		cy.clearCookies();
		cy.visit(pageUrl(issuePath) + '?reload=' + Date.now());
